...
void StakeReportDialog::setModel(WalletModel *model)
{
    ...
    if(ex_model && ex_model->getOptionsModel() && !alreadyConnected)
    {
        ...
        disablereportupdate = GetBoolArg("-disablereportupdate",0);
...
{
    QString result = WayawolfcoinUnits::format(nUnit, amount);

    int poin = result.indexOf(".") + 1;
    poin += WayawolfcoinUnits::decimals(nUnit);
...
void StakeReportDialog::updateStakeReport(bool fImmediate=false)
{
    ...
    ui->L_Coin->setText(WayawolfcoinUnits::name(nDisplayUnit) + " " + tr("SubTotal"));

    QTableWidget *TableW = ui->StakeReportTable;

    TableW->horizontalHeaderItem(1)->setText(WayawolfcoinUnits::name(nDisplayUnit) + " " +tr("Amount"));
    ...
    ui->Amount_24H->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" [WW]"));
    ui->Stake_24H->setText(QString::number(aRange[i++].Count));
    ui->Amount_7D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" [WW]"));
    ui->Stake_7D->setText(QString::number(aRange[i++].Count));
    ui->Amount_30D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" [WW]"));
    ui->Stake_30D->setText(QString::number(aRange[i++].Count));
    ui->Amount_365D->setText(Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" [WW]"));
    ui->Stake_365D->setText(QString::number(aRange[i++].Count));

    ui->Amount_Last->setText(tr("Amount: ") + Coin_0Pad(nDisplayUnit, aRange[i].Total) + tr(" [WW]"));
