...
{
    QString ret = QString("wayawolfcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non WW unit input in lnReqAmount, we generate the URI with WW as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(WayawolfcoinUnits::format(WayawolfcoinUnits::WW, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
