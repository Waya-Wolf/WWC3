...
int nCoinbaseMaturity = 9;// Blocks to confirm if mining pow pos
int nStakeMinConfirmations =  1440; // 1440 Blocks is 3 Days Target Minimum Maturity
unsigned int nStakeMinAge =  3 * 60 * 60 * 24; // 3 Days Target Minimum Maturity
unsigned int nModifierInterval = 3 * 60; // time to elapse before new modifier is computed

CBlockIndex* pindexGenesisBlock = NULL;
int nBestHeight = -1;
static const int64_t nTargetTimespan =  30 * 60; //30 Minute 10 Block Retagret
...
const string strMessageMagic = "Wayawolfcoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight) {
	double nSubsidy = (nHeight < PREMINE_PERIOD) ? INITIAL_SUBSIDY :
					  ...
					  std::pow(0.5, std::floor((nHeight - FIRST_HALVING_HEIGHT) / HALVING_INTERVAL)) * INITIAL_REWARD ;
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees, int nHeight) {
	double nSubsidy = (nHeight <= FIRST_HALVING_HEIGHT) ? 0 :
					  std::pow(0.5, std::floor((nHeight - FIRST_HALVING_HEIGHT) / HALVING_INTERVAL)) * INITIAL_REWARD ;
...
unsigned int GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	int64_t PastBlocksMin = 30; //2 hr min
	int64_t PastBlocksMax = 50; //3 hr max
	...
	const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
	...
	// fMiner is true when called from the internal Wayawolfcoin miner
...
{
	RenameThread("wayawolfcoin-loadblk");
